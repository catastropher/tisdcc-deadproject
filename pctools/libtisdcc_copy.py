...
def compile(codeobj, options):
  ...
  try:
    ...
    proc = Popen(["sdcc", "-mz80", "--opt-code-size", \
        "--stack-auto", "--std-sdcc99","--reserve-regs-iy"] + \
...
def link(codeobjlist, options, startupobjdir):
  ...
  try:
    ...
    nameobj.writetofile("%s/name.rel" % wd, silent=True)

    srcfiles = []
    for codeobj in codeobjlist:
      osfh, tempsrc = tempfile.mkstemp(suffix=".rel", dir=wd)
      os.close(osfh)
      fh = open(tempsrc, "wb")
      fh.write(codeobj.contents)
      fh.close()
    ...
    outfile = srcfiles[0].replace(".rel", ".ihx")
    extraflags = []
    if options.libpaths:
      for libp in options.libpaths:
        extraflags.append("-L%s" % libp)
    ...
    program = ["sdcc","--reserve-regs-iy", "-mz80", "--no-std-crt0", "--code-loc", \
        PLATFORMDATA[target]['codeloc'], "--data-loc", \
        PLATFORMDATA[target]['dataloc']] + \
        ["-Wl" + startupobjfile1, "-Wlname.rel", "-Wl" + startupobjfile2] + \
        extraflags + srcfiles
    proc = Popen(program)
    stdout, stderr = proc.communicate()
...
def compileonly(codeobj, options):
  ...
  try:
    ...
    proc = Popen(["sdcc","--reserve-regs-iy", "-mz80", "--opt-code-size", \
